...
## Regression to establish a (causal) relationship {-}
...
## Regression to adjust for pretreatment differences {-}
...
ggplot(data=data, aes(x=xx, y=yy)) +
  geom_point(mapping = aes(col=z)) + ylab('outcome') + xlab('pre-treatment') +
  geom_abline(intercept = coef(lm_2)[1], slope = coef(lm_2)[2], col='black') +
  geom_abline(intercept = coef(lm_2)[1] + coef(lm_2)[3],
              slope = coef(lm_2)[2], col='blue')
```
```{r, echo=FALSE}
...
We note that the peacekeeping study and the gun-control study are of the same type, but the peacekeeping study was more focused and was able to approximately account for systematic differences which does not seem possible with the gun-control study.
...
Three concerns for model building: *Information*, *Assumptions* and *Interpretation*:
