...
```{r}
# assign each roll to a random fold
...
for( i in 1:10)
{
  ...
  for(j in 1:nrow(testdat))
  {
    ...
    # is this the tidy way?
    newdat$score <- log(mean(dnorm(pred - kidiq$kid_score[i], mean = 0, sd = sigma)))
    kidiq |> rows_update(newdat, by = 'row_id')
    
    # they way I did it before.
   # kidiq[kidiq$row_id == newdat$row_id, "score"] <-
    #  log(mean(dnorm(pred - kidiq$kid_score[i], mean = 0, sd = sigma))) 
  }
}
```
