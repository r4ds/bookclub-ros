...
for( i in 1:10)
{
  ...
  {
    pred <- posterior_linpred(fit,newdata = testdat[j,])
    # how can i do this tidy?
    kidiq[kidiq$row_id == testdat[j,]$row_id,"score"] <- log(mean(dnorm(pred - kidiq$kid_score[i], mean = 0, sd = sigma))) 
  }
}
```
