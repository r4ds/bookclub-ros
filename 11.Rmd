...
* Naive implementation would require `n` fits, one for each held out data point.

* `rstan` has a function `loo` that can shortcut this using an approximation based on probability calculations.

* This function returns `elpd_loo` - expected log predictive density and it's standard error (amoung other things).  This is the sum of the log of the probability of outcome $y$ given predictors $x%. (Averaged over the parameter draws?)
